...
The properties a node can have. Node properties can be set and retrieved using the [InstanceGraphAPI](/api/webvis/interfaces/instancegraphapi).

Using [InstanceGraphAPI.setProperty](/api/webvis/interfaces/instancegraphapi#setproperty) will trigger a EventType.NODE\_CHANGED event.
...
\{ name: string \} & [AnimationOptions](/api/webvis/interfaces/animationoptions)
...
The node's variants can be accessed using [VariantsAPI.requestVariants](/api/webvis/interfaces/variantsapi#requestvariants).
...
The real property is used by the [RealityAPI](/api/webvis/interfaces/realityapi) to specify whether a node will be marked as tracking target for the Reality service.
...
the ViewerSettingStrings.RENDER\_MODE setting of the viewer but gets overridden by the [ViewerAPI.forceRenderMode](/api/webvis/interfaces/viewerapi#forcerendermode) method.
...
[Serializable](/api/webvis/types/serializable) | undefined
