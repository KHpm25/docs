...
Use the [InstanceGraphAPI](/api/webvis/interfaces/instancegraphapi) to **structure your data**, combine multiple sources, and define visibility:
...
Use the [SettingsAPI](/api/webvis/interfaces/settingsapi) to **configure viewer and session behavior** at runtime:
...
Use the [ViewerAPI](/api/webvis/interfaces/viewerapi) to **control how users move through the scene** by switching between navigation modes:
...
Use the [SelectionAPI](/api/webvis/interfaces/selectionapi) to **select & highlight nodes**, allowing inspection and connection with underlying data:
...
Use the [InstanceGraphAPI](/api/webvis/interfaces/instancegraphapi) to **control the visibility of individual nodes**:
...
Use the [InstanceGraphAPI](/api/webvis/interfaces/instancegraphapi) to **control how nodes are positioned**, oriented, and scaled within the scene:
...
Use the [InstanceGraphAPI](/api/webvis/interfaces/instancegraphapi) and [MaterialAPI](/api/webvis/interfaces/materialapi) to **adjust how your 3D data is presented**:
...
Use the [InstanceGraphAPI](/api/webvis/interfaces/instancegraphapi) to **visually emphasize nodes** and draw attention to relevant parts:
...
Use the [ViewerMagnifierAPI](/api/webvis/interfaces/viewermagnifierapi) to **provide a magnified view** of a region of interest within the viewport:
...
Use the [AnimationAPI](/api/webvis/interfaces/animationapi) to **animate nodes over time** and illustrate motion, sequences, or assembly steps:
...
Use the [ExplosionAPI](/api/webvis/interfaces/explosionapi) to **reveal internal structures** by separating geometry based on spatial relationships:
...
Use the [InstanceGraphAPI](/api/webvis/interfaces/instancegraphapi) to **add context** to your 3DSpace by accessing and using existing properties and metadata:
...
Use the [QueryAPI](/api/webvis/interfaces/queryapi) and the [LayerFilterAPI](/api/webvis/interfaces/layerfilterapi) to **find and isolate relevant content** within your 3DSpace:
...
Use the [InstanceGraphAPI](/api/webvis/interfaces/instancegraphapi) and [VariantsAPI](/api/webvis/interfaces/variantsapi) to **display Product Manufacturing Information (PMI)** and switch between **model views and variants**:
...
Use the [TopologyAPI](/api/webvis/interfaces/topologyapi) to **access B-Rep topology** such as faces, edges, and points:
...
Use the [MeasurementAPI](/api/webvis/interfaces/measurementapi) to perform **single, precise measurements** on faces, edges, and points directly in the 3DSpace:
...
Use the [InstanceGraphAPI](/api/webvis/interfaces/instancegraphapi) to **highlight differences between states or versions by color** for quick visual comparison:
...
Use the [SessionStorageAPI](/api/webvis/interfaces/sessionstorageapi) to **capture and restore states** of your 3DSpace to support workflows and reuse:
...
Use the [AnnotationAPI](/api/webvis/interfaces/annotationapi) to **add contextual information** directly to your 3DSpace to **support collaboration** and communication:
...
Use the [AttachmentAPI](/api/webvis/interfaces/attachmentapi) to **associate additional data with nodes** in your 3DSpace:
...
Use the [CollectionAPI](/api/webvis/interfaces/collectionapi) to **group nodes into collections** for efficient batch operations:
...
Use the [EventAPI](/api/webvis/interfaces/eventapi) to **react to changes in the 3DSpace** and build interactive applications:
...
Use the [CoordinateSystemAPI](/api/webvis/interfaces/coordinatesystemapi) to **work with coordinate systems** within your 3DSpace:
